...
week <- as.integer(1 +
  (Sys.Date() - as_date(start_semester))/7 -
  (Sys.Date() > as_date(mid_semester_break))
)
```
...
#| cache: false
schedule |>
  mutate(Topic = if_else(is.na(Topic), "", Topic)) |>
  transmute(
    Date = format(Date + 3, "%d %b"), # Thursday of each week
    Show = TRUE, #!is.na(Week) & Week <= week , # Only link to past and current weeks
